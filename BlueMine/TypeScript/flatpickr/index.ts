...
import { arrayify, debounce, int, pad, IncrementEvent } from "./utils/index";
import {
  clearNode,
  createElement,
  createNumberInput,
...
function FlatpickrInstance(
  ...
  function bind<E extends Element | Window | Document>(
    ...
    element.addEventListener(event, handler as EventListener, <boolean><any>options);
  ...
  function set<K extends keyof Options>(
    ...
    if (option !== null && typeof option === "object")
      ...
      self.config[<number><any>option] = value;

      if (CALLBACKS[<number><any>option] !== undefined)
        (CALLBACKS[<number><any>option] as Function[]).forEach(x => x());
    }
